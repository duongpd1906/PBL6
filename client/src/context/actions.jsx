export const DISPLAY_ALERT = "SHOW_ALERT";
export const CLEAR_ALERT = "CLEAR_ALERT";

export const REGISTER_USER_BEGIN = "REGISTER_USER_BEGIN";
export const REGISTER_USER_SUCCESS = "REGISTER_USER_SUCCESS";
export const REGISTER_USER_ERROR = "REGISTER_USER_ERROR";

export const LOGOUT_USER = "LOGOUT_USER";

export const LOGIN_USER_BEGIN = "LOGIN_USER_BEGIN";
export const LOGIN_USER_SUCCESS = "LOGIN_USER_SUCCESS";
export const LOGIN_USER_ERROR = "LOGIN_USER_ERROR";

export const GET_ALL_USERS_BEGIN = "GET_ALL_USERS_BEGIN";
export const GET_ALL_USERS_SUCCESS = "GET_ALL_USERS_SUCCESS";
export const GET_ALL_USERS_ERROR = "GET_ALL_USERS_ERROR";

export const GET_USER_PROFILE_BEGIN = "GET_USER_PROFILE_BEGIN";
export const GET_USER_PROFILE_SUCCESS = "GET_USER_PROFILE_SUCCESS";
export const GET_USER_PROFILE_ERROR = "GET_USER_PROFILE_ERROR";

export const GET_ALL_POSTS_BEGIN = "GET_ALL_POSTS_BEGIN";
export const GET_ALL_POSTS_SUCCESS = "GET_ALL_POSTS_SUCCESS";
export const GET_ALL_POSTS_ERROR = "GET_ALL_POSTS_ERROR";

export const CREATE_POST_BEGIN = "CREATE_POST_BEGIN";
export const CREATE_POST_SUCCESS = "CREATE_POST_SUCCESS";
export const CREATE_POST_ERROR = "CREATE_POST_ERROR";

export const UPDATE_AVATAR_BEGIN = "UPDATE_AVATAR_BEGIN";
export const UPDATE_AVATAR_SUCCESS = "UPDATE_AVATAR_SUCCESS";
export const UPDATE_AVATAR_ERROR = "UPDATE_AVATAR_ERROR";

export const SEND_INVITATION_BEGIN = "SEND_INVITATION_BEGIN";
export const SEND_INVITATION_SUCCESS = "SEND_INVITATION_SUCCESS";
export const SEND_INVITATION_ERROR = "SEND_INVITATION_ERROR";

export const ACCEPT_INVITATION_BEGIN = "ACCEPT_INVITATION_BEGIN";
export const ACCEPT_INVITATION_SUCCESS = "ACCEPT_INVITATION_SUCCESS";
export const ACCEPT_INVITATION_ERROR = "ACCEPT_INVITATION_ERROR";

export const GET_CONVERSATION_BEGIN = "GET_CONVERSATION_BEGIN";
export const GET_CONVERSATION_SUCCESS = "GET_CONVERSATION_SUCCESS";
export const GET_CONVERSATION_ERROR = "GET_CONVERSATION_ERRORS";

export const GET_LIST_CONVERSATIONS_BEGIN = "GET_LIST_CONVERSATIONS_BEGIN";
export const GET_LIST_CONVERSATIONS_SUCCESS = "GET_LIST_CONVERSATIONS_SUCCESS";
export const GET_LIST_CONVERSATIONS_ERROR = "GET_LIST_CONVERSATIONS_ERRORS";

export const GET_COMMENTS_OF_POST_BEGIN = "GET_COMMENTS_OF_POST_BEGIN";
export const GET_COMMENTS_OF_POST_SUCCESS= "GET_COMMENTS_OF_POST_SUCCESS";
export const GET_COMMENTS_OF_POST_ERROR = "GET_COMMENTS_OF_POST_ERROR";

export const GET_COMMENTS_OF_COMMENT_BEGIN = "GET_COMMENTS_OF_COMMENT_BEGIN";
export const GET_COMMENTS_OF_COMMENT_SUCCESS= "GET_COMMENTS_OF_COMMENT_SUCCESS";
export const GET_COMMENTS_OF_COMMENT_ERROR = "GET_COMMENTS_OF_COMMENT_ERROR";

export const COMMENT_POST_BEGIN = "COMMENT_POST_BEGIN";
export const COMMENT_POST_SUCCESS= "COMMENT_POST_SUCCESS";
export const COMMENT_POST_ERROR = "COMMENT_POST_ERROR";

export const CREATE_LIKE_BEGIN = "CREATE_LIKE_BEGIN";
export const CREATE_LIKE_SUCCESS= "CREATE_LIKE_SUCCESS";
export const CREATE_LIKE_ERROR = "CREATE_LIKE_ERROR";